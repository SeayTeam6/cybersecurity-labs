
# Active Directory Azure Lab

This lab demonstrates how to deploy and configure an Active Directory domain in Microsoft Azure. Active Directory (AD) is a system used to manage users, computers, and resources in a network. In this lab, we create a domain controller, join a client computer to the domain, create users and organizational units, and apply security settings. Each step includes explanations and instructions so even someone unfamiliar with AD or Azure can follow along.

---

## Objectives

- Deploy a Windows Server domain controller in Azure  
- Configure DNS and join a Windows client to the domain  
- Create Organizational Units for administrative and employee separation  
- Provision standard and privileged user accounts  
- Automate bulk user creation using PowerShell  
- Configure Group Policy security settings  
- Apply and validate account lockout enforcement  
- Perform account unlocks and password resets for users  
- Review authentication logs to track login attempts  

---

# Lab Walkthrough

## 1. Azure Environment Setup

**Explanation:**  
Before using Active Directory, we need a virtual environment in Azure. A resource group is a container for resources like virtual networks and virtual machines (VMs). A virtual network allows our servers and clients to communicate privately. Assigning a static IP ensures the domain server always has the same address, which is critical for DNS resolution. Temporarily disabling the firewall allows us to test connectivity. The client VM needs its DNS pointed to the domain server so it can locate and join the domain.

---

## Turn off firewalls in domain VM
Created a resource group, virtual network, and subnet in Azure. Deployed a Windows Server VM that will act as the domain server, assigned it a static private IP, and disabled the firewall for connectivity testing.  
<img width="526" height="546" alt="1  Turned off firewalls in domain VM" src="https://github.com/user-attachments/assets/6cbed85a-dcd9-47da-b589-ad1d1e624041" />

## Set Client VM DNS settings to the Domain VMs private address
Deployed a second Windows VM as the client in the same region and virtual network. Its DNS settings were pointed to the domain server's private IP.  
<img width="780" height="458" alt="2  Set Client VM DNS settings to the Domain VMs private address" src="https://github.com/user-attachments/assets/d574b3d8-7797-46cd-9751-d2232c62db43" />

## Ping Domain server from client to verify connectivity
Restarted the client VM and verified connectivity by pinging the domain server and confirming DNS configuration in PowerShell.  
<img width="858" height="701" alt="3  Pinged Domain server from client" src="https://github.com/user-attachments/assets/860e1eb3-df67-4f84-8dad-1c4cc06e95e8" />

---

## 2. Installing Active Directory

**Explanation:**  
Active Directory Domain Services (AD DS) allows centralized management of users, computers, and resources. Installing AD DS and promoting a server to a domain controller (DC) creates the core server that stores the directory database and handles authentication. A forest is the top-level container in Active Directory; it can contain one or more domains. Here we create a new forest called `mydomain.com`.

---

## Open Server Manager from the Start menu
Logged into the domain server VM and opened Server Manager.  
<img width="858" height="724" alt="4  go to the domain server press start menu and click server manager" src="https://github.com/user-attachments/assets/cca9644f-34e5-4707-90bf-79ef1d4fae9b" />

## Add Roles and Features
<img width="574" height="279" alt="5  Add roles and features" src="https://github.com/user-attachments/assets/0ba0b58f-ce61-4f02-8286-9107951a7b75" />

## Click next twice, select Active Directory Domain Services, and install
<img width="783" height="561" alt="6  Active directory domain services" src="https://github.com/user-attachments/assets/8c4f60eb-efba-46e5-bb18-c822b6a712cc" />

## Promote this server to a domain controller
Clicked the yellow flag in the top right and selected "Promote this server to a domain controller".  
<img width="647" height="325" alt="7  Click the flag in the top right and click promote this server to  a domain controller" src="https://github.com/user-attachments/assets/ac766924-55a1-47a6-b5c2-ad5c53c24c08" />

## Add a new forest and type the domain name
Selected "Add a new forest" and typed `mydomain.com`.  
<img width="757" height="553" alt="8  Add a new forest and type in the domains name" src="https://github.com/user-attachments/assets/60722f51-6919-40d2-9ddd-913d3794507d" />

## Create a DSRM password and complete installation
Entered a Directory Services Restore Mode (DSRM) password. Clicked next through remaining prompts and installed. VM restarted automatically.  
<img width="757" height="558" alt="9  create and enter a password" src="https://github.com/user-attachments/assets/db6d1136-3ddb-4343-a597-93db67fad09b" />

---

## 3. Active Directory Users and Organizational Units

**Explanation:**  
Organizational Units (OUs) help organize users, groups, and computers in Active Directory. This structure allows policies to be applied to specific sets of objects. `_EMPLOYEES` will hold standard users, and `_ADMINS` will hold administrative accounts. Proper OU organization is a best practice in professional IT environments.

---

## Open Active Directory Users and Computers
After restarting, logged in using the domain account and searched for Active Directory Users and Computers.  
<img width="1280" height="723" alt="10  Active directory Users and computers" src="https://github.com/user-attachments/assets/3c1f6fa1-52c9-4144-b55f-2e788719b2fe" />

## Right click domain, select New → Organizational Unit
<img width="755" height="529" alt="11  File New Organizational Unit" src="https://github.com/user-attachments/assets/07721769-b375-4090-8757-3a4916c9f259" />

## Create _EMPLOYEES OU
<img width="750" height="530" alt="12  Type in Employees" src="https://github.com/user-attachments/assets/a844551c-ccb7-40bc-b961-d74d5f76215f" />

## Create _ADMINS OU
<img width="752" height="527" alt="13  Repeat last step and type in ADMINS" src="https://github.com/user-attachments/assets/60eee859-39aa-4895-9a2d-5a351ba2d742" />

---

## 4. Admin User Creation

**Explanation:**  
Creating a domain admin account allows management of all domain resources. Adding the user to the Domain Admins group grants full administrative rights. This demonstrates privilege management in AD.

---

## Create a new user in _ADMINS
Right-click _ADMINS → New → User and fill out the details.  
<img width="756" height="611" alt="14  Created a new user in admins" src="https://github.com/user-attachments/assets/224292fd-e691-4670-9fe6-b48a0c67e593" />
<img width="751" height="528" alt="15  Created a new user in admins  2" src="https://github.com/user-attachments/assets/406690bf-9898-4ae3-8dbd-8521963285a5" />

## Create a password
<img width="747" height="526" alt="16  Create a Password" src="https://github.com/user-attachments/assets/dce1b4e9-bd1c-4ce9-baf6-4b68a50827f1" />

## Add user to Domain Admins
Right-click user → Properties → Member Of → Add → Type "Domain Admins" → Check Names → OK → Apply → OK.  
<img width="750" height="835" alt="17  Right click username, member of, Add" src="https://github.com/user-attachments/assets/dbd0610e-f69c-407e-a80d-9f5049f03fdd" />
<img width="752" height="837" alt="18  Type in Domain Admins, Check names, Then OK" src="https://github.com/user-attachments/assets/a7c5a992-0709-49ae-8b88-ccc2ea020855" />

## Log in as the new Admin
<img width="463" height="435" alt="19  Login as the new admin account" src="https://github.com/user-attachments/assets/c7063b75-9644-4776-be0e-448160260dae" />

---

## 5. Joining Client to Domain

**Explanation:**  
Joining a client computer to the domain allows centralized management and authentication. The client must use the domain controller's IP as its DNS. Once joined, domain accounts can log in, and policies from AD apply to the client.

---

## Open System settings on CLIENT1
Right-click Start Menu → System → Rename this PC → Change.  
<img width="308" height="613" alt="20  Login into CLient-1 vm and right click start menu and click system" src="https://github.com/user-attachments/assets/93b4c7a7-40f9-48c7-a6e6-3b3bbc95f7be" />
<img width="1213" height="508" alt="21   Click rename this pc advanced then under computer name chick change" src="https://github.com/user-attachments/assets/c03c613c-e906-4d72-8c6b-cd2e87fb5284" />

## Join the client to the domain
Click Domain → Type `mydomain.com` → Enter Admin credentials → OK → Restart.  
<img width="411" height="422" alt="22  joining the client into the Domain Click domain and type in mydomaincom" src="https://github.com/user-attachments/assets/8f65876a-d1f1-47b5-bfb1-f574ce632a2c" />
<img width="455" height="300" alt="23  Enter the admins username and password and OK then restart" src="https://github.com/user-attachments/assets/42aa0773-da54-43f8-b200-41ad1dcda2bb" />
<img width="299" height="151" alt="24  Admin is now added to the domain" src="https://github.com/user-attachments/assets/0113fa73-2c02-4cc0-ab77-43864c9e7aea" />


---

## 6. Client Organizational Unit Management

**Explanation:**  
Placing client computers into an OU helps organize machines and allows Group Policies to be applied to specific groups. `_CLIENTS` will contain all client machines, keeping the directory structured and making policy management easier.

---

## Create a new _CLIENTS OU and move client computers into it
On the DC-1 VM, open Active Directory Users and Computers. Right-click the domain → New → Organizational Unit → `_CLIENTS`. Drag all client computers from the default "Computers" container into the new `_CLIENTS` OU.  
<img width="753" height="527" alt="25  make a new organizational unit in dc1 users and computers drag clients computers to new client folder" src="https://github.com/user-attachments/assets/d8532513-937b-43f6-9be4-11b4bae58082" />

---

## 7. Remote Desktop Configuration

**Explanation:**  
Configuring Remote Desktop access for domain users allows authorized users to connect to the client computers. By adding the Domain Users group, all domain users can log in, demonstrating central permission management.

---

## Log into CLIENT VM as admin
<img width="408" height="489" alt="26  log into the CLIENT vm as the admin user" src="https://github.com/user-attachments/assets/8641e28d-81a9-4e95-8dad-ee9261e9319c" />

## Open System settings → Remote Desktop
Navigate to Start Menu → System → Remote Desktop.  
<img width="866" height="877" alt="27  Start System Remote desktop" src="https://github.com/user-attachments/assets/f87e1a7f-3428-4f33-b954-0b288923024b" />
<img width="972" height="688" alt="28  Select users that can remotely access this pc" src="https://github.com/user-attachments/assets/3b205ad6-3244-4347-b404-fdba334625f8" />

## Add Domain Users group
Type `Domain Users` → Check Names → OK. This allows all users in the domain to log in to this computer.  
<img width="665" height="533" alt="29  type in domain users then check names" src="https://github.com/user-attachments/assets/0660c5c4-9f4c-4588-b6fa-e6ac9d057cca" />

---

## 8. Bulk User Creation via PowerShell

**Explanation:**  
Using PowerShell to create multiple users automates account creation and simulates real-world scenarios where many employees need accounts. This demonstrates combining scripting with AD management.

---

## Open PowerShell ISE as administrator on DC1
Create a new script file and paste the content for user creation.  
<img width="793" height="595" alt="30  Open Powershell ISE as administrator in DC1 and paste script" src="https://github.com/user-attachments/assets/8838e600-8566-4a38-805f-75369e8befa0" />

## Press the play/start button to run the script
This will create 1000 users in the `_EMPLOYEES` OU.  
<img width="798" height="599" alt="31  Press the play start button and allow the script to run to create 1000 users" src="https://github.com/user-attachments/assets/42e8a7c6-22fe-4df5-a398-a4b94de42d8" />

---

## 9. Testing Employee Logins

**Explanation:**  
Logging in with newly created users ensures accounts were correctly added and replicated. This confirms domain authentication and access to client computers.

---

## Select an employee account in _EMPLOYEES OU
<img width="749" height="599" alt="32  choose an employee that was created by the script" src="https://github.com/user-attachments/assets/aa5f47f7-b119-48b2-bafc-320bfccd1440" />

## Log into CLIENT1 with the created account
<img width="409" height="497" alt="33  Log into the CLIENT1 with one of the created accounts" src="https://github.com/user-attachments/assets/8154dc97-22db-44a4-bc0d-409c133d0b9f" />
<img width="471" height="486" alt="34  Log into the CLIENT1 with one of the created accounts" src="https://github.com/user-attachments/assets/e1c3089f-7688-4592-9e2e-a56754e17fff" />

## Confirm login success
<img width="611" height="443" alt="35  ban wel is succesfully logged in" src="https://github.com/user-attachments/assets/921bd09f-736d-46c4-84af-9b06b260045c" />

---

## 10. Account Lockout Policy Configuration

**Explanation:**  
Account lockout policies prevent brute-force attacks by locking a user account after a number of failed login attempts. Configuring this via Group Policy ensures enforcement across the domain.

---

## Open Group Policy Management Console (GPMC)
Run `gpmc.msc` and navigate to Default Domain Policy.  
<img width="412" height="225" alt="36  set up an Account Lockout policy in Active Directory using the Group Policy Management Console" src="https://github.com/user-attachments/assets/ab9cadfd-10f9-4bed-a963-4672a5027561" />

## Edit Default Domain Policy
Right-click Default Domain Policy → Edit.  
<img width="749" height="531" alt="37  Right click Default domain policy then click edit" src="https://github.com/user-attachments/assets/dc070bda-ecb9-463a-ac10-d63cdae015ae" />

## Set Account Lockout Threshold
Navigate to Computer Configuration → Policies → Windows Settings → Security Settings → Account Policies → Account Lockout Policy. Set threshold to 5 failed login attempts.  
<img width="866" height="694" alt="38  Computer configuration - policies-windos settings-security settings- account policies- account lockout policy then click threshhold and set to 5" src="https://github.com/user-attachments/assets/9b9b5adb-7a47-419e-a15d-0d21c7628c18" />

---

## 11. Force Policy Update on Client

**Explanation:**  
After configuring Group Policy, clients must refresh to apply the policy immediately.

---

## Run gpupdate /force on CLIENT1
Log into CLIENT1 as an admin (e.g., `jane_admin`) → Run Command Prompt as Administrator → `gpupdate /force`. Then `gpupdate /r` to see updated policies.  
<img width="628" height="976" alt="39  force the domain policy to update using command line in client 1" src="https://github.com/user-attachments/assets/38f3dfe0-99be-4239-9868-b1019268770e" />

---

## 12. Test Account Lockout and Reset

**Explanation:**  
Failing login multiple times triggers lockout. Unlocking and resetting the account demonstrates administrative control in AD.

---

## Trigger account lockout
Logout of admin, log in with a new employee account multiple times with incorrect passwords to lock it.  
<img width="453" height="409" alt="40  Now logout of the admin client and log back into CLIENT1 using the new user and  fail to login to force a lockout" src="https://github.com/user-attachments/assets/35e07d1d-14be-43f9-a390-6f619669f9d8" />
<img width="557" height="145" alt="41  Locked out" src="https://github.com/user-attachments/assets/7008c529-3f66-42e8-8a19-78e285a06f1b" />

## Unlock account in Active Directory Users and Computers
Right-click `mydomain` → Find → Enter locked account → Properties → Account → Check "Unlock account" → Apply.  
<img width="1137" height="630" alt="43  unlock the account" src="https://github.com/user-attachments/assets/3a92ff1f-7e03-4279-91f9-1158d5ed9fc1" />

## Confirm user can log in
<img width="613" height="573" alt="44  Logged in" src="https://github.com/user-attachments/assets/f6bb05ac-14ed-4c44-871a-a5889d8a4767" />
<img width="518" height="198" alt="45  Ran whoami comandline in powershell to show that i have logged in" src="https://github.com/user-attachments/assets/49979cea-8205-4738-897a-0a1df9791a26" />

## Reset user password
Right-click user → Reset Password → Enter new password → OK.  
<img width="518" height="386" alt="47  Reset password" src="https://github.com/user-attachments/assets/b073ad40-408f-41ff-955d-a30659bd4cd8" />

---

## 13. Review Event Logs for Authentication

**Explanation:**  
Event Viewer allows admins to review login successes, failures, and account activities. This is critical for auditing and security monitoring.

---

## Open Event Viewer on DC1
Start → Run → `eventvwr.msc`. Navigate to Windows Logs → Security.  
<img width="776" height="783" alt="48  Go to DC1 VM click start and type in eventvwr msc to see authentication logs" src="https://github.com/user-attachments/assets/457342f4-dd00-4825-9e6b-16dcb330b81a" />

## Search logs for a specific account
Right-click Security → Find → Enter username → Find Next. Logs will highlight login attempts with details.  
<img width="860" height="547" alt="49  Right click security and click find" src="https://github.com/user-attachments/assets/1877afa8-8ee6-4680-bcf8-2d5f5b40fa38" />
<img width="860" height="970" alt="50  Type in account name and click Find Next  Logs will be highlighted of that account with information of the log at the bottom screen" src="https://github.com/user-attachments/assets/ea8afd34-3e10-4418-a541-6ba55ad2c30a" />

## Review failed login attempts on CLIENT1
Run Event Viewer as admin on CLIENT1 → Windows Logs → Security → Audit Failures. Details include account name, date/time, and endpoint.  
<img width="1099" height="1015" alt="51  Log of failed login in event viewer" src="https://github.com/user-attachments/assets/17e64e07-2c0f-4f52-8e96-47a5df0ef8c0" />


This lab has demonstrated how to:

- Deploy and configure Active Directory in Azure  
- Create and manage users and OUs  
- Join client machines to the domain  
- Configure Remote Desktop and permissions  
- Automate user creation via PowerShell  
- Apply account lockout policies and reset accounts  
- Monitor authentication using Event Viewer  














